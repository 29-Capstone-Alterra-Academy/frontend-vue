<template>
  ...
  >
    ...
    <v-snackbar v-model="snackbarReject" :timeout="5000">
      ...
      <template #action="{ attrs }">
        <v-btn color="primary" text v-bind="attrs" @click="snackbarReject = false"
          >Close</v-btn
        >
      </template>
    </v-snackbar>
    ...
    <v-card>
      ...
      <v-divider></v-divider>
      <v-card-text class="">
        Yakin ingin memblokir postingan ini?
      </v-card-text>
      <v-card-actions>
        <v-list-item>
          <v-list-item-content>
...
export default {
  ...
  methods: {
    acceptReport() {
      this.$axios
        .put(
          `/topic/${this.thread.topic.id}/report?scope=thread&threadId=${this.thread.thread.id}&reporterId=${this.thread.reporter.id}&action=approve`,
          {},
          {
            headers: {
    ...
    rejectReport() {
      this.$axios
        .put(
          `/topic/${this.thread.topic.id}/report?scope=thread&threadId=${this.thread.thread.id}&reporterId=${this.thread.reporter.id}&action=reject`,
          {},
          {
            headers: {
              Authorization: 'Bearer ' + this.$store.state.auth.accessToken,
